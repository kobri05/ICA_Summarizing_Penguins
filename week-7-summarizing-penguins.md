...
``` r
penguins |>
  ggplot(mapping = aes(x = bill_ratio, y = species)) +
  geom_boxplot() +
  labs(x = "Bill Ratio (mm)", y = "Penguin Species")
```
...
penguins |> 
  ...
  ggplot(mapping = aes(x = bill_length_mm, y = species)) +
  geom_boxplot()
```
...
penguins |> 
  mutate(body_mass_kg = body_mass_g/1000) |> 
  group_by(species) |> 
  summarize(mean_body_mass_kg = mean(body_mass_kg, na.rm = TRUE),
            sd_body_mass_kg = sd(body_mass_kg, na.rm = TRUE))
```

    # A tibble: 3 × 3
      species   mean_body_mass_kg sd_body_mass_kg
      <fct>                 <dbl>           <dbl>
    1 Adelie                 3.70           0.459
...
penguins |>
  ...
  group_by(island) |> #I thought there was an error here because it only returned data from Dream island, but alas that is the only island where chinstrap penguins were measured.
  summarise(min_flipper_length = min(flipper_length_mm, na.rm = TRUE),
            max_flipper_length = max(flipper_length_mm, na.rm = TRUE))
```

    # A tibble: 1 × 3
      island min_flipper_length max_flipper_length
      <fct>               <int>              <int>
    1 Dream                 178                212
...
``` r
penguins |>
  select(c("species", "bill_ratio")) |>
  group_by(species) |>
  drop_na() |>
  summarise(mean = mean(bill_ratio))
```
...
```
    ...
    # A tibble: 3 × 4
      island    bill_length_mm_med bill_depth_mm_med flipper_length_mm_med
      <fct>                  <dbl>             <dbl>                 <dbl>
...
``` r
penguins |>
  filter(island == c("Dream", "Torgersen")) |>
  select(species:body_mass_g) |>
  rename(spp_penguin = species) |> #https://dplyr.tidyverse.org/reference/rename.html
...
``` r
penguins |>
  count(species, year) #counts the number of species surveyed per year in the dataset
```
...
penguins |> 
  ...
  group_by(species) |> #groups data by spcies
  mutate(total = sum(n),  #creates a new column with the total number of each species in the dataset
         prop = n/total) |>  #calculates the proportion of each island compared to the total number of each species 
  ggplot(aes(x = island, y = prop, fill = species)) + #starts ggplot and establishes x value, y value, and fill
  geom_col(position = position_dodge()) + #creates the column graph
  scale_fill_viridis_d() + #color fill scale
...
``` r
penguins |>
  select(species, year, flipper_length_mm) |>  #selecting the columns species, year, and flipper length
  rename(study_year = year) |>   #renames year to study_year
  filter(species == "Adelie") |>  #filters for Adelie penguins only
  mutate(flipper_rank = case_when(   #creates a new column that ranks flipper length; rank '1' when flipper length is less than 200 mm, rank '2' when flipper length is greater than or equal to 200 mm, and rank 0 if anything else (i.e. NA)
    flipper_length_mm < 200 ~ 1,   #creates flipper rank '1' when flipper length is less than 200 mm
    flipper_length_mm >= 200 ~ 2,  #creates flipper rank '2' when flipper length is greater than or equal to 200 mm
